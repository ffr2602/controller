...
from controller.robot import compute_motor_velocities
...
class BaseController(Node):
    def __init__(self):
        ...
        if (self.declare_parameter('arduino_port').value):
            ...
            self.get_logger().info(f"Setting arduino_port: {arduino_port}")

        if (self.declare_parameter('baudrate').value):
            baudrate = self.get_parameter('baudrate').value
            self.get_logger().info(f"Setting wheel base: {baudrate}")
        ...
        try:
            ...
            print(f"Unable to open connection on serial port:{arduino_port}")

        self.create_subscription(Twist, '/cmd_vel', self.cmd_vel_cb, qos_profile=qos_profile_system_default)
        self.odom_pub = self.create_publisher(Odometry, '/odom', qos_profile=qos_profile_services_default)
        ...
        while True:
            ...
            if (self.serial_connected):
                try:
                    ...
                    self.get_logger().info(f"Send data: {encoded_data}")
